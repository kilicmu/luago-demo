...
const (
	IABC  = iota // => A B C bytes: 8,9,9
	IABx         // => A Bx bytes: 8, 18
	IAsBx        // => A sBx bytes：8, 18
	IAx          // => Ax bytes: 26
)
...
const (
	...
	OpArgR        // => argument is a registrer or a jump offset
...
type opcode struct {
	...
	argBMode byte // B argument mode
	argCMode byte // C argument mode
	opMode   byte // op mode
	name     string
}
...
const MAXAGE_BX = 1<<18 - 1
const MAXAGE_SBX = MAXAGE_BX >> 1

func (self Instruction) AsBx() (a, sbx int) {
	a, bx := self.ABx()
	return a, bx - MAXAGE_SBX
}
